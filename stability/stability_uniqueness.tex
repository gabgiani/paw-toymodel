\documentclass[pra,twocolumn,amsmath,amssymb,longbibliography]{revtex4-2}
...
\title{Stability and Uniqueness of Tensor Product Structures\\ under Hamiltonian Dynamics:\\
A Mathematical Framework for Dynamically Selected Factorizations\\
...
\keywords{tensor product structure, Page--Wootters formalism,
...
\end{equation}
with $p(t) = \Tr_{SE}\!\bigl[\langle t|_C\,\rho_{CSE}\,|t\rangle_C\bigr]$.
...
\[
  ...
  \;\subset\; \End_0(\hilb),
\]
where $\End_0(\hilb) := \{X\in\End(\hilb):\Tr(X)=0\}$.
\end{definition}
...
\begin{align}\label{eq:Delta1}
  \Delta_1 &= -i\Bigl(
    ...
    &\quad - \rho_S\otimes\Tr_S[H_{\mathrm{int}},\sigma]
  \Bigr) \nonumber\\
...
\begin{multline}\label{eq:effective}
...
\item PW formula $\Longrightarrow$ presupposes $S|E$\hfill (Sec.~\ref{sec:setup})
\item Stability of $S|E$ $\Longrightarrow$
  $I(S{:}E)(t)\le Kt^2\|H_{\mathrm{int}}\|_2^2$\hfill (Thm.~\ref{thm:quadratic})
\item $\|H_{\mathrm{int}}\|_2\le\eta\|H\|_2$ $\Longrightarrow$
  $\eta$-stable\hfill (Def.~\ref{def:stability})
\item Two $\eta$-stable bipartitions $\Longrightarrow$
  $\theta=O(\eta)$\hfill (Thm.~\ref{thm:angle})
\item $\theta=O(\eta)$ $\Longrightarrow$
  $\|P_{\calA_0}-P_{\calA_0'}\|=O(\eta)$\hfill (Lem.~\ref{lem:summand})
\item Close algebras $\Longrightarrow$
  $U\approx U_S\otimes U_E$\hfill (Lem.~\ref{lem:conjugation})
\item Uniqueness $\Longrightarrow$
  $\calF(S|E)$ variational principle\hfill (Thm.~\ref{thm:variational})
\item Optimal bipartition $\Longrightarrow$
  almost-unitary $\rho_S(t)$\hfill (Thm.~\ref{thm:dynamics})
\end{enumerate}
...
\item $K,c_\sigma,C_\sigma$ --- Quadratic bound (Thm.~\ref{thm:quadratic}):
  depends on spectrum of $\rho_S,\rho_E$.
\item $4$ (as in $4\varepsilon$) --- Lemma~\ref{lem:summand}:
  none (universal).
\item $C_1\le 5$ --- Orthonormalization
  (Lem.~\ref{lem:conjugation}, Step~2):
  for $\delta<1/4$.
\item $C_2,C_3$ --- Almost-multiplicativity (Step~3):
  $d_S$ (structure constants of $\mathfrak{su}(d_S)$).
\item $C_5,\ldots,C_8$ --- Idempotent/isometry correction
  (Step~4): $d_S$.
\item $\lambda_\star=d_S$ --- Casimir gap (Step~6):
  $d_S$ (representation theory).
\item $C(d_S,d_E)$ --- Final constant
  (Lem.~\ref{lem:conjugation}):
