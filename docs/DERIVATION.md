...
## 15. Clock Orientation Covariance — Derivation

The twelve preceding sections derived the Unified Formula and showed that it produces the three pillars.  Here we derive a structural symmetry of the formula itself: its covariance under arbitrary permutations of the clock basis.

### Setup

Consider the history state in its block decomposition:

|Ψ⟩ = (1/√N) Σ_{k=0}^{N-1} |k⟩_C ⊗ |ψ_k⟩_{SE}

where |ψ_k⟩_{SE} = U_{SE}(k dt)|ψ₀⟩.  In practice, vectorizing the state gives a column vector whose first d_{SE} entries correspond to clock tick k = 0, the next d_{SE} entries to k = 1, and so on.  In NumPy:

```
blocks = psi_vec.reshape(N, d_SE)
phi_k = blocks[k, :]          # conditional SE state at tick k
```

### Step 1: Permuting the clock basis

Let π ∈ S_N be any permutation.  The "permuted" history state is the same physical state |Ψ⟩, but we now read it using clock basis {|π(0)⟩, |π(1)⟩, …, |π(N−1)⟩}.  Projecting onto slot j of the permuted basis:

⟨π(j)|_C |Ψ⟩ = (1/√N) |ψ_{π(j)}⟩_{SE}

This is a **pure reindexing**: slot j of the permuted basis picks up block π(j) of the original decomposition.

### Step 2: Normalization

p^π(j) = ‖⟨π(j)|_C |Ψ⟩‖² = (1/N) ‖|ψ_{π(j)}⟩‖² = p(π(j))

The probabilities are invariant under relabelling (expected from Born rule).

### Step 3: Partial trace

ρ_S^π(j) = Tr_E[ |ψ_{π(j)}⟩⟨ψ_{π(j)}| ] / (N · p(π(j)))

But this is identically ρ_S(π(j)) — the state one would obtain by conditioning on tick π(j) in the original basis.

### Result

**ρ_S^π(j) = ρ_S(π(j))** for all π ∈ S_N, for all j.

The Unified Formula's three-step structure (projection → normalization → partial trace) is covariant under all N! relabellings of the clock.  No dynamical assumption is invoked; this is a kinematical identity of the conditioning procedure.

### Special case: reversal

For π_R(k) = N−1−k:

- Pillar 1: ρ_S^R(j) = ρ_S(N−1−j), recovering states in reverse order → reversed Schrödinger dynamics
- Pillar 2: S_eff^R(j) = S_eff(N−1−j) → perfect arrow inversion
- Pillar 3: locality metrics are invariant → the reversed observer's frame is self-consistent

### Why clock reversal ≠ T-reversal

Clock reversal: slot j ↦ block N−1−j → state U((N−1−j)dt)|ψ₀⟩

T-reversal: state → state*, effectively U(−jdt)|ψ₀⟩ = U(jdt)†|ψ₀⟩

Since (N−1−j)dt ≠ −jdt for any finite N, these are algebraically distinct.  T-reversal acts on the **state** (anti-unitarily); clock reversal acts on the **conditioning index** (unitarily/combinatorially).

---

## 16. Angular Interpolation — Continuous Clock Rotation

Section 15 treated the clock basis as a discrete permutation of labels.  Here we extend this to a **continuous one-parameter family** that interpolates smoothly between forward and reversed temporal orderings.

### Construction

The key observation is that the reversal permutation π_R(k) = N−1−k pairs each clock tick k with its time-reversed partner N−1−k.  Within each such pair, we can perform a continuous rotation:
...
where θ ∈ [0, π].  This is a standard 2×2 rotation matrix applied to the pair (|k⟩, |N−1−k⟩).

### Step 1: Orthonormality

Since the rotation acts within each 2D subspace independently, and distinct subspaces are automatically orthogonal, the full set {|k_θ⟩}_{k=0}^{N−1} is orthonormal for all θ:

⟨j_θ|k_θ⟩ = δ_{jk}  ∀ θ

Numerically verified at 5 test angles: max|⟨j_θ|k_θ⟩ − δ_{jk}| < 3×10⁻¹⁷.

### Step 2: Conditioning on the rotated basis

Projecting |Ψ⟩ onto |k_θ⟩_C:

⟨k_θ|_C |Ψ⟩ = cos(θ/2)·⟨k|_C|Ψ⟩ + sin(θ/2)·⟨N−1−k|_C|Ψ⟩

= (1/√N)[cos(θ/2)·|ψ_k⟩_{SE} + sin(θ/2)·|ψ_{N−1−k}⟩_{SE}]

This is a **coherent superposition** of the SE states at two different times.  Unlike a statistical mixture, it preserves interference between the forward and reversed evolution.

### Step 3: The conditioned density matrix

ρ_S^θ(k) = Tr_E[ |φ_k^θ⟩⟨φ_k^θ| ] / p^θ(k)

where |φ_k^θ⟩ = ⟨k_θ|_C|Ψ⟩.  Expanding:

ρ_S^θ(k) ∝ cos²(θ/2)·ρ_S(k) + sin²(θ/2)·ρ_S(N−1−k) + cross-terms

The **cross-terms** (proportional to sin(θ)·Tr_E[|ψ_k⟩⟨ψ_{N−1−k}|]) are the temporal interference contributions.  They vanish at θ = 0 and θ = π (where only one time contributes) and are maximal near θ = π/2.

### Step 4: Arrow strength as a function of θ

Define A(θ) = [S_eff(N−1, θ) − S_eff(0, θ)] / ln 2, measuring the entropy difference between the last and first clock tick at angle θ.  Results:

| θ | A(θ) | Monotonicity M(θ) |
|---|------|--------------------|
| 0 | +0.9995 | 1.000 |
| π/4 | +0.4499 | 0.621 |
| 0.365π (= θ*) | 0.000 | ~0.39 |
...
The function A(θ) is continuous, smooth, and strictly decreasing.  By the Intermediate Value Theorem, there exists a unique θ* where A(θ*) = 0.  Numerically, θ* ≈ 0.365π.

### Why θ* ≠ π/2

If the initial state |ψ₀⟩ were symmetric under time reversal, we would expect θ* = π/2.  But |ψ₀⟩ = |↑⟩ is not time-reversal symmetric (it selects a particular spin direction), so the forward evolution creates stronger entanglement growth than the reversed evolution "de-grows" it.  The zero-crossing of A(θ) is displaced toward smaller θ.

This asymmetry is a **geometric invariant** of the initial state's position relative to the time-reversal operation in the Bloch ball.

### Boundary consistency

At θ = 0: |k_0⟩ = |k⟩, so ρ_S^0(k) = ρ_S(k) — the standard forward conditioning.

At θ = π: |k_π⟩ = |N−1−k⟩ (up to sign), so ρ_S^π(k) = ρ_S(N−1−k) — the reversed conditioning.

Both boundaries reproduce the results of the Covariance Theorem (Section 15) as special cases.
...
python generate_covariance_theorem.py   # Covariance theorem + T-symmetry
